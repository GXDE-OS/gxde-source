...
install:
	...
	if [ "$(GXDE_CODENAME)" = "zhuangzhuang" ]; then cp -rv 90-gxde-lizhi-backports $(DESTDIR)/etc/apt/preferences.d/90-gxde-lizhi-backports ; fi
	if [ "$(GXDE_CODENAME)" = "lizhi" ]; then cp -rv 90-gxde-lizhi-backports $(DESTDIR)/etc/apt/preferences.d/90-gxde-lizhi-backports ; fi
